...
//            xyseries.add((double) temp.getTime().getTime(),Double.parseDouble(temp.getData())); // 对应的横轴
    ...
    {  // 统计异常点的长度
        ...
        //添加数据值
        int predictPeriod = abnor.getLength() - 1;
        xyseries.add(norDataLength-predictPeriod -1, Double.parseDouble(abnor.getElementAt(abnor.getLength() - 1).getData()));//预测长度设置
        for (int i = 0; i < length - 1; i++) {

            DataItem temp=new DataItem();
            temp=abnor.getElementAt(i);
            xyseries.add(i+norDataLength-predictPeriod,Double.parseDouble(temp.getData()));
        }
        xyseriescollection.addSeries(xyseries);
        return xyseriescollection;
    }
