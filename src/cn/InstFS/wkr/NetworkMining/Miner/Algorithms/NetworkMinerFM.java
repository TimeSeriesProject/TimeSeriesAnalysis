...
//			return;
		...
		if(task.getMiningAlgo().equals(MiningAlgo.MiningAlgo_NeuralNetworkTSA)){
			...
			//p.setPredictPeriod(predictPeriod);
			tsaMethod=new NeuralNetwork(task.getSourcePath()+task.getMiningObject(), dataItems.getLastTime(), task,p);
		}else if(task.getMiningAlgo().equals(MiningAlgo.MiningAlgo_ARIMATSA)){
			p.setPredictPeriod(10);
			tsaMethod=new ARIMATSA(task, dataItems, p);
		}else{
