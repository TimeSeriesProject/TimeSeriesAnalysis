...
import cn.InstFS.wkr.NetworkMining.Miner.Common.KSTestTable;
 ...
 */
public class KolmogorovSmirnovCheckout {
     ...
     */
    public KolmogorovSmirnovCheckout() {
        rng = new Well19937c();
    }
     ...
     */
    public KolmogorovSmirnovCheckout(RandomGenerator rng) {
        this.rng = rng;
    }
    ...
    public boolean kolmogorovSmirnovTest(RealDistribution distribution, double[] data, double alpha) {
        ...
        KSTestTable table = new KSTestTable(data.length, alpha);
        double D_na = table.getD_na(); //查表值 
        double D_sup = kolmogorovSmirnovStatistic(distribution, data);  //ks统计量
        return D_sup < D_na;
    }
