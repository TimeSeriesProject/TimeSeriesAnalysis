	...
	private void getlinkList()
	{
		...
		try
		{
			for(int i=0;i<srcFiles.list().length;i++)
			{
				...
				String curLine="";
				String header =textReader.readLine(); //�ļ�ͷ��
				while((curLine=textReader.readLine())!=null)
				{
					Map<Integer,Double>	map = new HashMap<Integer,Double>();  //��¼���ƽ������
...
//				 System.out.println("matrix "+matrix.size());
	...
	public DataItems readInputByText()
	{
		...
		{
		case "��ϵ��": return readClusterByText();
		case "����ֱ��":return readDiameterByText();
		case "�ڵ�仯":return readNodeFrequenceByText();
		default: return null;
		}
	}
	@Override
